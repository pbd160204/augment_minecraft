...
execute if score @s ag_aug27 matches 1.. run tellraw @s {"text":"[채굴] 돌도 씹어 먹을 나이","color":"aqua","hover_event":{"action":"show_text","value":"Y좌표가 0 이하일 때 지속적으로 포화 효과를 획득합니다."}}
execute if score @s ag_aaug1 matches 1.. run tellraw @s {"text":"[고급] 원코","color":"yellow","hover_event":{"action":"show_text","value":"PvP 단계 사망 시 1회 인벤토리를 유지한 채 즉시 부활하며 최대 체력 10, 15초 투명화와 10초 신속 I을 얻고 무작위 위치로 이동합니다."}}
execute if score @s ag_aaug2 matches 1.. run tellraw @s {"text":"[고급] 박사 과정","color":"green","hover_event":{"action":"show_text","value":"모든 경험치를 60% 더 많이 획득하지만 허기 I 효과를 받습니다."}}
...
execute if score @s ag_aug30 matches 1.. run tellraw @s {"text":"[퀘스트] 정글러","color":"white","hover_event":{"action":"show_text","value":"강타 촉매로 여섯 재료를 바쳐 늑대 갑옷과 재생 I을 가진 체력 100·공격력 8의 정글펫을 소환합니다."}}
...
execute if score @s ag_aug32 matches 1.. run tellraw @s {"text":"[채굴] 황금 고블린","color":"aqua","hover_event":{"action":"show_text","value":"특수 금 장비 상점을 해금합니다. 경기 중 노란색 발광하며, 처치자는 당신을 처치하면 경험치 2000을 직접 획득합니다."}}
execute if score @s ag_aug33 matches 1.. run tellraw @s {"text":"[전투] 양자역학","color":"red","hover_event":{"action":"show_text","value":"크기가 45% 감소하며 공격 피해가 2 감소합니다."}}
...
execute if score @s ag_aug36 matches 1.. run tellraw @s {"text":"[생존] 로렌츠 힘","color":"yellow","hover_event":{"action":"show_text","value":"월드보더 경계 5블록 이내에서 신속 II를 얻습니다."}}
execute if score @s ag_aug37 matches 1.. run tellraw @s {"text":"[마법사] 아제리유제","color":"light_purple","hover_event":{"action":"show_text","value":"저울 2개로 가장 가까운 상대와 레벨을 비교합니다."}}
